...
\begin{equation}
    \E{W} = \frac{1}{\pi} \sum_{i < j}W_{ij}\text{Pr}\left[\text{sgn}\left(\dotp{\vec{v}_i,
    \vec{r}}\right) \neq \text{sgn}\left(\dotp{\vec{v}_j,
    \vec{r}}\right)\right].
\end{equation} They then state the following lemma:
\begin{lemma}
    \begin{equation}
        \text{Pr}\left[\text{sgn}\left(\dotp{\vec{v}_i,
        \vec{r}}\right) \neq \text{sgn}\left(\dotp{\vec{v}_j,
        \vec{r}}\right)\right] = \frac{1}{\pi} \arccos(\dotp{\vec{v}_i,
        \vec{v}_j}),
    \end{equation}
