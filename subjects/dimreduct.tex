...
\TODO{Read
https://www.science.org/doi/full/10.1126/science.290.5500.2319?casa\_token=whDQpzuoiBkAAAAA\%3AYzbhWSgYlY6xTx399b0zDK7OLnSRkdrdKcp8024KOfx-nETbTfJXHeM80U06Phop5o5K7eSPI2Jm0g}

We now give an overview of dimensionality reduction (DR) in machine learning
scenarios. This overview looks at how DR has been employed in both input
and parameter reduction techniques. We still seek to find more papers on
DR in parameter reduction tasks, specifically in reducing the communication
costs in parameter update methods such that the relative structure of the
updates are preserved. Both techniques are being reviewed for intuition.

Classical, linear methods are outlined in Sections
\ref{sec:random_projections} and \ref{sec:principle_component_analysis}.
...
\paragraph*{Random Projection Algorithm}
\begin{enumerate}
    \item Start with a matrix $\mat A \in \R^{m \times n}$ and a low-dimensional
        embedding $\mat \Pi \in \R^{k \times n}$. 
    \item Project the matrix into the low-dimensional embedding $\mat Y = \mat A
        \mat \Pi^\intercal$. 
    \item Find an orthonormal basis $\mat Q$ that captures the column space of
        $\mat Y$, i.e. some matrix which captures the information of the
        projection. 
    \item Find the closest point to $\mat A$ under this change of basis, i.e.
        some matrix $\mat X$ which minimizes $\norm{\mat Q^\intercal \mat A -
        \mat X}_F$ while having a low rank.
\end{enumerate}
...
Let $\mat Y^\mppi$ denote the Moore-Penrose pseudo-inverse.
...
\begin{remark}
    ...
    own intuition, I believe $s$ to be the \emph{rank-reduction parameter} while $k$ is
    the \emph{dimensionality-reduction parameter}. The parameters respectfully represent
...
\TODO{Include Gaussian low-rank approximation from
\cite{improved-matrix-algorithms-via-the-subsampled-randomized-hadamard-transform}
since it seems promising.}
...
\begin{theorem}
    Fix a rank-reduction parameter $2 \leq s < r$. Let $0 < \epsilon < 1/3$ be an accuracy
    ...
    \begin{align}
        \label{eq:}
        \norm{\mat A - \mat Y \mat Y^\mppi \mat A}_F &\leq (1 + 22\epsilon) \norm{\mat A - \mat A_k}_F,\\
        \norm{\mat A - \tilde{\mat A}_k}_F &\leq (1 + 22\epsilon) \norm{\mat A - \mat A_k}_F,\\
        \norm{\mat A_k - \mat Y \mat Y^\mppi \mat A}_F &\leq (1 + 22\epsilon) \norm{\mat A - \mat A_k}_F,\\
    ...
    \begin{align}
        \norm{\mat A - \mat Y \mat Y^\mppi \mat A}_F &\leq \left(4 + \sqrt{\frac{3 \ln (n / \delta) \ln(r / \delta)}{k}}\right) \norm{\mat A - \mat A_k} + \sqrt{\frac{3 \ln(r / \delta)}{k}} \norm{\mat A - \mat A_k}_F\\ 
        ...
        \norm{\mat A_k - \mat Y \mat Y^\mppi \mat A}_F &\leq \left(4 + \sqrt{\frac{3 \ln (n / \delta) \ln(r / \delta)}{k}}\right) \norm{\mat A - \mat A_k} + \sqrt{\frac{3 \ln(r / \delta)}{k}} \norm{\mat A - \mat A_k}_F\\ 
    ...
    \mat Y^\mppi \mat A$ can be constructed in $\cal{O}(mnl + mkl)$, and
...
\label{sec:principle_component_analysis}

\section{Comparisons}

\cite{experiments-with-random-projections-for-machine-learning} states that PCA
often has a higher computational complexity, but tends to outperform RP. RP
benefits from having a lower computational complexity as well as maintaining
distance-preserving properties. Both PCA and RP are tested across decision
tress, nearest neighbor methods, and SVM on two-class classification problems
using a projection built s.t. $r_{i, j} \sim \cal{R}$ where $\cal{R}$ is the
Rademacher distribution. Their results show that both algorithms can nearly
match the performance of the original algorithm for nearest neighbor problems,
but RP underperforms PCA on almost all accounts for the other two problems.

\cite{dimensionality-reduction-a-comparative} shows that nonlinear  methods
(e.g.\ auto-encoders) underperform traditional PCA on 1-nearest neighbor
classification tasks for artificial and natural datasets alike (often
drastically). Even the best non-convex methods seem to only match the
performance of PCA. All nonlinear methods tend to have a much higher
computational complexity than PCA with limited exception.
