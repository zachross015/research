...
scenarios. Classical, linear methods are outlined in Sections
...
underperform traditional PCA on 1-nearest neighbor classification tasks for 
artificial and natural datasets alike (often drastically). Even the best
non-convex methods seem to only match the performance of PCA, while all
nonlinear methods tend to have a much higher computational complexity than PCA.
...
distance-preserving properties.

\cite{experiments-with-random-projections-for-machine-learning} tests both PCA
and RP on decision tress, nearest neighbor methods, and SVM on two-class
classification problems using a projection built s.t. $r_{i, j} \sim \cal{R}$
where $\cal{R}$ is the Rademacher distribution. Their results show that both
algorithms can nearly match the performance of the original algorithm for
nearest neighbor problems, but RP underperforms PCA on almost all accounts for
