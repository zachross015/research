...
$\mat A_s = \mat U_s \mat \Sigma_s \mat V_s^\intercal$. Furthermore, let $k \leq
n$, let $l = \min\set{m, k}$ and let $\mat Q \mat R = \mat A \mat \Pi^\intercal $ be the QR
decomposition of this projection s.t.\ $\mat Q \in \R^{m \times l}$ and $\mat
Q^\intercal \mat Q = \mat I_l$. Note that the column space of $\mat A \mat
\Pi^\intercal$ is contained in the range of $\mat Q$. Let 
\begin{equation}
...
\begin{remark}
    ...
    A}_s$ minimizes the projection in to the low-dimensional embedding. Note
...
\begin{definition}
    \cite{improved-matrix-algorithms-via-the-subsampled-randomized-hadamard-transform}
    Let $n = 2^n$. The \emph{Non-normalized Walsh-Hadamard Transform} is a block matrix of
...
\begin{theorem}
    ...
    \begin{align}
        ...
        \norm{\mat A_k - \tilde{\mat A}_k}_F &\leq \left(7 + \sqrt{\frac{12 \ln (n / \delta) \ln(r / \delta)}{k}}\right) \norm{\mat A - \mat A_k} + \sqrt{\frac{6 \ln(r / \delta)}{k}} \norm{\mat A - \mat A_k}_F
    \end{align}
\end{theorem}
